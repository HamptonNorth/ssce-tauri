/**
 * Text Tool Property Card
 *
 * Properties:
 * - Colour (palette + custom + eyedropper)
 * - Text Size (XS/SM/MD/LG presets)
 */
...
export class TextCard extends BasePropertyCard {
  constructor() {
    super("text", "Text");
  ...
  renderContent() {
    ...
    const currentSize = state.textSize || getToolConfig("text", "size") || "md";
    ...
    this.textSizeGroup = createTextSizeGroup({
      value: currentSize,
      onChange: (size) => this.handleTextSizeChange(size),
    });
    sizeSection.appendChild(this.textSizeGroup.element);
    this.registerComponent(this.textSizeGroup);
  ...
  handleColourChange(colour) {
    ...
    setToolConfig("text", "colour", colour);
  }
  ...
  handleTextSizeChange(size) {
    // Update global state (textSize is used by text, steps, symbols tools)
    state.textSize = size;
...
/**
 * Create a text size button group
 ...
 */
function createTextSizeGroup(options = {}) {
  const { value = "md", onChange = () => {} } = options;

  // Get text sizes from centralized config
